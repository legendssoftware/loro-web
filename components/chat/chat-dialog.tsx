                ...
                <DialogHeader className="p-4 flex flex-row items-center justify-between border-b">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <DialogTitle className="text-sm font-body uppercase font-normal">
                                LORO GUY
                            </DialogTitle>
                ...
                <ScrollArea className="flex-1 px-4">
                    <div className="space-y-4 py-4">
                        {messages.map((message) => (
                            ...
                            >
                                <Avatar className="h-10 w-10">
                                    <AvatarFallback className={`${message.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-primary/10 text-primary'} text-[10px] font-body uppercase`}>
                                        {message.sender === 'user' ? 'SG' : 'LG'}
                                    </AvatarFallback>
                                </Avatar>
                                <div className={`flex flex-col gap-1 max-w-[80%] ${message.sender === 'user' ? 'items-end' : ''}`}>
                                    <div className={`rounded-xl p-3 ${message.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'} font-body font-thin text-xs`}>
                                        {message.text}
                                    </div>
                                </div>
                            </div>
