...
export function RouteGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const allowedRoutes = rolePermissions[userRole as AccessLevel]?.routes || [];

      // Check for wildcards or exact route matches
      hasAccess = allowedRoutes.includes('*') || allowedRoutes.includes(pathname);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && isAuthenticated && !isLoading && pathname === '/') {
