...
import { ClaimResponse, CreateClaimDTO, UpdateClaimDTO } from '@/lib/types/claims';
...
export const fetchClaims = async (config: ClaimsRequestConfig): Promise<ClaimResponse> => {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/claims?${queryParams.toString()}`, {
            headers: {
...
export const fetchClaimByRef = async (ref: number, config: RequestConfig) => {
    ...
    try {
        const response = await axios.get<ClaimResponse>(`${API_URL}/claims/${ref}`, {
            headers: {
...
export const fetchClaimsByUser = async (ref: number, config: RequestConfig) => {
    ...
    try {
        const response = await axios.get<ClaimResponse>(`${API_URL}/claims/for/${ref}`, {
            headers: {
                Authorization: `Bearer ${config.headers.token}`,
                'Content-Type': 'application/json',
            },
...
export const createClaim = async (claim: CreateClaimDTO, config: RequestConfig) => {
    ...
    try {
        const response = await axios.post<ClaimResponse>(`${API_URL}/claims`, claim, {
            headers: {
                Authorization: `Bearer ${config.headers.token}`,
                'Content-Type': 'application/json',
            },
...
export const updateClaim = async ({
    ...
    try {
        const response = await axios.patch<ClaimResponse>(`${API_URL}/claims/${ref}`, updatedClaim, {
            headers: {
                Authorization: `Bearer ${config.headers.token}`,
                'Content-Type': 'application/json',
            },
...
export const deleteClaim = async (ref: number, config: RequestConfig) => {
    ...
    try {
        const response = await axios.delete<ClaimResponse>(`${API_URL}/claims/${ref}`, {
            headers: {
                Authorization: `Bearer ${config.headers.token}`,
                'Content-Type': 'application/json',
            },
