...
export const fetchLeads = async (config: RequestConfig): Promise<LeadResponse> => {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/leads?${queryParams.toString()}`, {
            headers: {
...
export const restoreLead = async (ref: number, config: RequestConfig) => {
    ...
    try {
        const response = await axios.patch<LeadResponse>(
            `${API_URL}/leads/restore/${ref}`,
            {},
            {
                headers: {
                    Authorization: `Bearer ${config.headers.token}`,
