...
export function useForgotPasswordMutation() {
  return useMutation({
    mutationFn: async ({ email }: { email: string }): Promise<ForgotPasswordResponse> => {
      try {
...
export function useResetPasswordMutation() {
  return useMutation({
    mutationFn: async ({ token, password }: { token: string; password: string }): Promise<ResetPasswordResponse> => {
      try {
...
export function useSetPasswordMutation() {
  return useMutation({
    mutationFn: async ({ token, password }: { token: string; password: string }): Promise<SetPasswordResponse> => {
      try {
        const { data } = await axios.post(`${API_URL}/auth/set-password`, { token, password });
        return data;
      } catch (error) {
        if (axios.isAxiosError(error)) {
