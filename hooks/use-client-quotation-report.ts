...
export function useClientQuotationReport(
  ...
  const [filters, setFilters] = useState<ClientQuotationReportFilters>(initialFilters);
  ...
  const {
    ...
    queryFn: async () => {
      console.log('Fetching client quotation report with filters:', filters);
  ...
  const updateFilters = (newFilters: Partial<ClientQuotationReportFilters>) => {
    setFilters(prev => ({
      ...prev,
      ...newFilters
    }));
