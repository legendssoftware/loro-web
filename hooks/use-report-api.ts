...
export const useReportApi = () => {
  ...
  const fetchClientQuotationReport = useCallback(
    async (clientId: number, params: {
      startDate?: string;
      endDate?: string;
      name?: string;
      additionalFilters?: Record<string, any>;
    } = {}): Promise<ReportResponse<any>> => {
      console.log(`API Call: Fetching quotation report for client ${clientId}`, params);
      try {
        ...
        console.log(`API Success: Received report data for client ${clientId}`, {
        ...
        console.error(`API Error: Failed to fetch report for client ${clientId}:`, error);
        return {
          data: null,
