...
import { formatPrice } from '@/lib/utils/currency-formatter';
import {
    X,
    Calendar,
    Clock,
...
export function ClaimDetailsModal({
    ...
    const [pendingStatusChange, setPendingStatusChange] = useState<ClaimStatus | null>(null);
    const [currentStatus, setCurrentStatus] = useState<ClaimStatus>(claim?.status || ClaimStatus.PENDING);
            ...
            case 'details':
                return (
                    <div className='space-y-6'>
                        ...
                        {claim?.documentUrl && (
                            <div className='p-4 rounded-lg bg-card/50'>
                                <h3 className='mb-2 text-xs font-normal uppercase font-body'>Attached Document</h3>
                        ...
                        <div className='p-4 rounded-lg bg-card/50'>
                            <h3 className='mb-2 text-xs font-normal uppercase font-body'>Claim Details</h3>
                            <div className='grid grid-cols-2 gap-4'>
                                <div>
                                    <p className='text-[10px] uppercase text-muted-foreground font-body'>Amount</p>
                                    <p className='text-sm font-thin font-body'>{claim?.amount}</p>
                                </div>
                                <div>
                                    <p className='text-[10px] uppercase text-muted-foreground font-body'>Category</p>
                                    <p className='text-sm font-thin font-body'>{claim?.category}</p>
                                </div>
                                <div>
                                    <p className='text-[10px] uppercase text-muted-foreground font-body'>Created Date</p>
                                    <p className='text-sm font-thin font-body'>{formatDate(claim?.createdAt)}</p>
                                </div>
                                {claim?.verifiedAt && (
                                    <div>
                                        <p className='text-[10px] uppercase text-muted-foreground font-body'>Verified Date</p>
                                        <p className='text-sm font-thin font-body'>{formatDate(claim?.verifiedAt)}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                ...
                                {claim?.verifiedAt && (
                                    <div className='flex items-center justify-between'>
                                        <div className='flex items-center'>
                                            ...
                                            <span className='text-[10px] font-thin uppercase font-body'>Verified</span>
                                        </div>
                                        <span className='text-xs font-thin font-body'>
                                            {formatDate(claim?.verifiedAt)} {formatTime(claim?.verifiedAt)}
                                        </span>
                                    </div>
                                )}

                                {claim?.updatedAt && formatDate(claim?.updatedAt) !== formatDate(claim?.createdAt) && (
                                    <div className='flex items-center justify-between'>
                                        <div className='flex items-center'>
                                            <Pencil className='w-4 h-4 mr-2 text-card-foreground/60' />
                                            <span className='text-[10px] font-thin uppercase font-body'>Last Updated</span>
                                        </div>
                                        <span className='text-xs font-thin font-body'>
                                            {formatDate(claim?.updatedAt)} {formatTime(claim?.updatedAt)}
                                        </span>
                                    </div>
                                )}
                            </div>
            ...
            case 'people':
                return (
                    <div className='space-y-6'>
                        {claim?.owner && (
                            <div className='p-4 rounded-lg bg-card/50'>
                                <h3 className='mb-2 text-xs font-normal uppercase font-body'>Claim Owner</h3>
                                <div className='flex items-center gap-3 p-3 border rounded-md border-border/30'>
                                    <Avatar className='w-10 h-10 border border-primary/30'>
                                        <AvatarImage src={claim.owner.photoURL} />
                                        <AvatarFallback className='text-xs font-thin uppercase font-body'>
                                            {getOwnerInitials()}
                                        </AvatarFallback>
                                    </Avatar>
                                    <div className='space-y-0.5'>
                                        <p className='text-sm font-medium font-body'>{claim.owner.name} {claim.owner.surname || ''}</p>
                                        <p className='text-xs font-thin text-muted-foreground font-body'>{claim.owner.email}</p>
                                        {claim.owner.phone && (
                                            <p className='text-xs font-thin text-muted-foreground font-body'>{claim.owner.phone}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        {claim?.branch && (
                            <div className='p-4 rounded-lg bg-card/50'>
                                ...
                                <div className='p-3 border rounded-md border-border/30'>
                                    <p className='text-sm font-medium font-body'>{claim.branch.name}</p>
                                    <p className='text-xs font-thin text-muted-foreground font-body'>{claim.branch.email}</p>
                                    <p className='text-xs font-thin text-muted-foreground font-body'>{claim.branch.phone}</p>
                                    {claim.branch.address && (
                                        <div className='mt-2'>
                                            <p className='text-[10px] uppercase text-muted-foreground font-body'>Address</p>
                                            <p className='text-xs font-thin font-body'>
                                                {[
                            ...
                            <div className='relative pl-8 space-y-8 before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-green-500 dark:before:bg-green-600'>
                                ...
                                <div className='relative'>
                                    ...
                                    <div className='flex flex-col'>
                                        <p className='text-xs font-normal uppercase font-body'>Claim created</p>
                                        ...
                                        <p className='text-[10px] font-normal text-card-foreground/60 uppercase font-body'>
                                            Created by {claim?.owner?.name || 'System'}
                                        </p>
                                    </div>
                                </div>
                                ...
                                {claim?.updatedAt && formatDate(claim?.updatedAt) !== formatDate(claim?.createdAt) && (
                                    <div className='relative'>
                                        ...
                                        <div className='flex flex-col'>
                                            <p className='text-xs font-normal uppercase font-body'>Claim updated</p>
                                            <p className='text-xs font-thin uppercase text-card-foreground/50 font-body'>
                                                {formatDate(claim?.updatedAt)} {formatTime(claim?.updatedAt)}
                                            </p>
                                            <p className='text-[10px] font-normal text-card-foreground/60 uppercase font-body'>
                                ...
                                {claim?.verifiedAt && (
                                    <div className='relative'>
                                        ...
                                        <div className='flex flex-col'>
                                            <p className='text-xs font-normal uppercase font-body'>Claim verified</p>
                                            <p className='text-xs font-thin uppercase text-card-foreground/50 font-body'>
                                                {formatDate(claim?.verifiedAt)} {formatTime(claim?.verifiedAt)}
                                            </p>
                                        </div>
    ...
    const EditModal = () => (
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-card">
                <DialogHeader>
                    <DialogTitle className="text-xl font-semibold uppercase font-body">Edit Claim</DialogTitle>
                </DialogHeader>
        ...
        return (
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-card">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-semibold uppercase font-body">Claim Details</DialogTitle>
                    </DialogHeader>
                    <div className="flex items-center justify-center h-64">
        ...
        <>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-card">
                    <DialogHeader className="flex flex-row items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button variant="ghost" size="icon" className="w-9 h-9" onClick={handleEdit}>
                    ...
                    <DialogFooter className="flex flex-col flex-wrap gap-4 pt-4 mt-6 border-t border-border/30">
                        <div className="flex flex-col items-center justify-center w-full">
                            <p className="text-xs font-thin uppercase font-body">Manage this claim</p>
                        </div>
                        <div className="flex flex-wrap justify-center w-full gap-3">
                            <Button
                                ...
                                onClick={() => handleStatusChange(ClaimStatus.PENDING)}
                                title="Set as Pending"
                            >
                                <AlertCircle strokeWidth={1.2} className="text-yellow-600 w-7 h-7" />
                            </Button>
                            <Button
                                variant="outline"
                                size="icon"
                            ...
                            >
                                <CheckCircle strokeWidth={1.2} className="text-green-600 w-7 h-7" />
                            </Button>
                            <Button
                                variant="outline"
                                size="icon"
                            ...
                            >
                                <Ban strokeWidth={1.2} className="text-red-600 w-7 h-7" />
                            </Button>
                            <Button
                                variant="outline"
                                size="icon"
                                className={`w-14 h-14 rounded-full ${getStatusButtonVariant(ClaimStatus.PAID)}`}
                                onClick={() => handleStatusChange(ClaimStatus.PAID)}
                                title="Set as Paid"
                            >
                                <CreditCard strokeWidth={1.2} className="text-blue-600 w-7 h-7" />
                            </Button>
                            <Button
                                variant="outline"
                                size="icon"
                                className={`w-14 h-14 rounded-full ${getStatusButtonVariant(ClaimStatus.CANCELLED)}`}
                                onClick={() => handleStatusChange(ClaimStatus.CANCELLED)}
                                title="Set as Cancelled"
                            >
                                <CalendarX2 strokeWidth={1.2} className="text-gray-600 w-7 h-7" />
                            </Button>
                            <Button
                                variant="destructive"
                                size="icon"
            ...
            <AlertDialog open={confirmStatusChangeOpen} onOpenChange={setConfirmStatusChangeOpen}>
                <AlertDialogContent className="bg-card">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="font-semibold font-body">Confirm Status Change</AlertDialogTitle>
                        <AlertDialogDescription className="font-thin font-body">
                            Are you sure you want to change the claim status to {pendingStatusChange}? This action cannot
                            be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent className="bg-card">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="font-semibold font-body">Confirm Deletion</AlertDialogTitle>
                        <AlertDialogDescription className="font-thin font-body">
