...
export function ClaimsKanban({ claimsByStatus, onUpdateStatus, onDelete, onAddClaim }: ClaimsKanbanProps) {
            ...
            return (
                <div className='flex-1 min-w-[280px] max-w-[320px] flex flex-col'>
                    <div className='flex items-center justify-between mb-2'>
                        ...
                        <Button variant='ghost' size='icon' className='w-6 h-6' onClick={() => onAddClaim?.()}>
    ...
    return (
        <div className='flex flex-row items-start w-full h-full gap-2 overflow-x-scroll overflow-y-hidden'>
            {renderColumn(ClaimStatus.PENDING, 'Pending', claimsByStatus[ClaimStatus.PENDING]?.length || 0)}
            {renderColumn(ClaimStatus.APPROVED, 'Approved', claimsByStatus[ClaimStatus.APPROVED]?.length || 0)}
            {renderColumn(ClaimStatus.REJECTED, 'Rejected', claimsByStatus[ClaimStatus.REJECTED]?.length || 0)}
            {renderColumn(ClaimStatus.PAID, 'Paid', claimsByStatus[ClaimStatus.PAID]?.length || 0)}
            {renderColumn(ClaimStatus.CANCELLED, 'Cancelled', claimsByStatus[ClaimStatus.CANCELLED]?.length || 0)}
            {renderColumn(ClaimStatus.DECLINED, 'Declined', claimsByStatus[ClaimStatus.DECLINED]?.length || 0)}
        </div>
    );
}
