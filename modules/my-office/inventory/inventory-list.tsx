...
import { PeriodFilterValue, getDateRangeFromPeriod } from '@/modules/common/period-filter';
import { Button } from '@/components/ui/button';
import { CreateProductDTO, Product, RequestConfig, UpdateProductDTO } from '@/lib/types/products';
...
const InventoryListComponent = ({
    ...
    const filteredProducts = useMemo(() => {
        ...
        const filtered = products.data.filter(product => {
            const matchesStatus =
                statusFilter === 'all' || product.status?.toUpperCase().trim() === statusFilter.trim();
            const matchesCategory = categoryFilter === 'all' || product.category?.trim() === categoryFilter.trim();
            ...
            const matchesSearch = searchTerm === '' || searchableFields.some(field => field.includes(searchTerm));
        ...
        return [...filtered].sort((a, b) => {
            ...
            return orderBy === 'asc' ? nameA.localeCompare(nameB) : nameB.localeCompare(nameA);
        });
    ...
    return (
        <motion.div variants={containerVariants} initial='hidden' animate='show' className='flex flex-col w-full gap-4'>
