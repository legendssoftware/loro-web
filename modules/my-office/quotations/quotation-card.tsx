...
const QuotationCardComponent = ({ quotation, onClick }: QuotationCardProps) => {
    ...
    return (
        <motion.div variants={itemVariants}>
            <Card className='transition-shadow cursor-pointer hover:shadow-md' onClick={() => onClick(quotation)}>
                <CardContent className='p-4'>
                    <div className='flex flex-col gap-2'>
                        <div className='flex items-center justify-between'>
                            ...
                            <span className='text-xs font-body'>#{quotation?.quotationNumber || 'N/A'}</span>
                        </div>
                        <div className='flex items-center justify-between'>
                            <span className='text-xs font-body text-muted-foreground'>
                                {format(quotationDate, 'MMM dd, yyyy')}
                            </span>
                            <span className={cn('text-xs font-body', amount > 0 ? 'text-green-600' : 'text-red-600')}>
                                R{amount.toFixed(2)}
                            </span>
                        </div>
