...
import { Building2, FolderOpen, List, TrendingUp, TrendingDown, Pencil, ChevronLeft, ChevronRight } from 'lucide-react';
...
const QuotationListComponent = ({ quotations, onQuotationClick, isLoading }: QuotationListProps) => {
    ...
    const filteredQuotations = useMemo(() => {
        const filtered = quotations.filter(quotation => {
            ...
            const matchesClient = clientFilter === 'all' || quotation.client.uid.toString() === clientFilter;
            const matchesUser = userFilter === 'all' || quotation.placedBy.uid.toString() === userFilter;
            ...
            return matchesStatus && matchesClient && matchesUser && matchesCategory && matchesPeriod;
        });
        ...
        return filtered;
    }, [quotations, statusFilter, clientFilter, userFilter, periodFilter, valueSort, categoryFilter]);
            ...
            <div className='flex items-center justify-end gap-2'>
                <div className='flex items-center gap-2'>
                        ...
                        <SelectContent>
                            <SelectItem value='all' className='text-[10px] font-normal uppercase font-body'>
                                <div className='flex flex-row items-center gap-2'>
                                    ...
                                    <span className='text-[10px] font-normal uppercase font-body'>All SKUs</span>
                                </div>
                            </SelectItem>
                            {uniqueSkus.map(sku => (
                                <SelectItem
                                ...
                                >
                                    <div className='flex flex-row items-center gap-2'>
                                        ...
                                        <span className='text-[10px] font-normal uppercase font-body'>{sku}</span>
                                    </div>
                                </SelectItem>
                            ))}
                        </SelectContent>
                    ...
                    <Select value={valueSort} onValueChange={(value: 'none' | 'asc' | 'desc') => setValueSort(value)}>
                        ...
                        <SelectContent>
                            <SelectItem value='none' className='text-[10px] font-normal uppercase font-body'>
                                <div className='flex flex-row items-center gap-2'>
                                    <List size={17} strokeWidth={1.5} />
                                    <span className='text-[10px] font-normal uppercase font-body'>Default Order</span>
                                </div>
                            </SelectItem>
                            <SelectItem value='asc' className='text-[10px] font-normal uppercase font-body'>
                                <div className='flex flex-row items-center gap-2'>
                            ...
                            <SelectItem value='desc' className='text-[10px] font-normal uppercase font-body'>
                                <div className='flex flex-row items-center gap-2'>
                            ...
                            {quotations
                                .filter(
                                    (quotation, index, self) =>
                                        index === self.findIndex(q => q.client.uid === quotation.client.uid),
                                )
                                .map(quotation => (
                                    <SelectItem
                    ...
                    <Select value={userFilter} onValueChange={setUserFilter}>
                        ...
                        <SelectContent>
                            ...
                            {quotations
                                .filter(
                                    (quotation, index, self) =>
                                        index === self.findIndex(q => q.placedBy.uid === quotation.placedBy.uid),
                                )
                                .map(quotation => (
                                    <SelectItem
    ...
    const toggleAllQuotations = () => {
        setSelectedQuotations(prev =>
            prev.length === filteredQuotations.length ? [] : filteredQuotations.map(q => q.uid),
        );
    };
