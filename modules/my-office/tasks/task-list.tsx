...
import { PeriodFilter, PeriodFilterValue } from '@/modules/common/period-filter';
...
const TaskListComponent = ({
    ...
    const filteredTasks = useMemo(() => {
        return tasks.filter(task => {
            ...
            // Client filter
            if (clientFilter !== 'all') {
                const hasClient = task.clients?.some(client => client.uid.toString() === clientFilter);
                if (!hasClient) return false;
            }

            // Assignee filter
            if (assigneeFilter !== 'all') {
                const hasAssignee = task.assignees?.some(assignee => assignee.uid.toString() === assigneeFilter);
        ...
        return (
            <div className='space-y-4'>
                ...
                <div className='flex flex-col items-center justify-center w-full gap-2 h-96'>
                    ...
                    <p className='text-[10px] font-normal uppercase text-muted-foreground font-body'>No tasks available</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className='flex flex-col w-full h-full gap-4'>
            ...
            <motion.div
                ...
                initial='hidden'
                animate='show'
                className='grid grid-cols-1 gap-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'
            >
