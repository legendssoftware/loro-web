...
import { TaskFilterParams, TaskPriority, TaskStatus, TaskType, Task } from '@/lib/types/task';
import {
    CalendarIcon,
    Check,
    ChevronDown,
...
export function TasksFilter({ onApplyFilters, onClearFilters, tasks = [] }: TasksFilterProps) {
    ...
    }, [search, status, priority, taskType, assigneeId, startDate, endDate, onApplyFilters]);
                ...
                task.assignees.forEach(assignee => {
                    if (assignee.uid && !assigneesMap.has(assignee.uid)) {
                        assigneesMap.set(assignee.uid, {
                            ...
                            avatar: assignee.photoURL || assignee.avatarUrl || '',
                        });
                    }
                });
            }
                    ...
                    <DropdownMenuTrigger asChild>
                        <div className='flex items-center justify-between w-full h-10 gap-2 px-3 border rounded cursor-pointer bg-card border-border'>
                            <div className='flex items-center gap-2'>
                                {priority ? (
                                    <>
                                        <Award className={`h-3 w-3 ${priorityColors[priority]}`} />
                                    </>
                                ) : (
                        ...
                        <DropdownMenuGroup>
                            {Object.values(TaskPriority).map(priorityOption => (
                                ...
                                >
                                    <div className='flex items-center gap-1'>
                                        <>
                                            <Award className={`h-4 w-4 ${priorityColors[priorityOption]}`} />
                                        </>
                                        <span
                                            className={`uppercase text-[10px] font-normal ml-1 ${
                                                priority === priorityOption ? priorityColors[priorityOption] : ''
                                            }`}
                                        >
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <div className='flex items-center justify-between w-full h-10 gap-2 px-3 border rounded cursor-pointer bg-card border-border'>
                            <div className='flex items-center gap-2'>
                                {taskType ? (
                                    <Tag className={`h-4 w-4 ${taskTypeColors[taskType]}`} />
                                ) : (
                    ...
                    <DropdownMenuContent align='start' className='p-1 w-[220px]'>
                        ...
                        <DropdownMenuGroup className='max-h-[300px] overflow-y-auto'>
                            {Object.values(TaskType).map(typeOption => {
                                let TypeIcon;
                                if (typeOption === 'IN_PERSON_MEETING') TypeIcon = Users;
                                else if (typeOption === 'VIRTUAL_MEETING') TypeIcon = Video;
                                else if (typeOption === 'CALL') TypeIcon = Phone;
                                else if (typeOption === 'EMAIL') TypeIcon = Mail;
                                else if (typeOption === 'WHATSAPP') TypeIcon = MessageSquare;
                                else if (typeOption === 'SMS') TypeIcon = MessageCircle;
                                else if (typeOption === 'FOLLOW_UP') TypeIcon = Redo2;
                                else if (typeOption === 'PROPOSAL') TypeIcon = FileText;
                                else if (typeOption === 'REPORT') TypeIcon = BarChart2;
                                else if (typeOption === 'QUOTATION') TypeIcon = FileText;
                                else if (typeOption === 'VISIT') TypeIcon = MapPin;
                                else TypeIcon = Tag;
                                ...
                                return (
                                    ...
                                    >
                                        <div className='flex items-center gap-1'>
                                            <TypeIcon className={`w-4 h-4 mr-2 ${taskTypeColors[typeOption]}`} />
                                            <span
                                                className={`uppercase text-[10px] font-normal ${
                                                    taskType === typeOption ? taskTypeColors[typeOption] : ''
                                                }`}
                                            >
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align='start' className='p-1 w-[180px]'>
                        ...
                        <DropdownMenuItem
                            className='flex items-center justify-between gap-2 px-2 rounded cursor-pointer h-9'
                            onClick={() => {
                                setAssigneeId(assigneeId === -1 ? undefined : -1);
                                setTimeout(handleApplyFilters, 0);
                            }}
                        >
                            <div className='flex items-center gap-2'>
                            ...
                            {assigneeId === -1 && <Check className='w-4 h-4 text-primary' />}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                            className='flex items-center justify-between gap-2 px-2 rounded cursor-pointer h-9'
                            onClick={() => {
                        ...
                        >
                            ...
                            {assigneeId === 0 && <Check className='w-4 h-4 text-primary' />}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        {assignees.map(assignee => (
                            <DropdownMenuItem
                                ...
                                className='flex items-center justify-between gap-2 px-2 rounded cursor-pointer h-9'
                                onClick={() => {
                                    setAssigneeId(assigneeId === assignee.id ? undefined : assignee.id);
                                    setTimeout(handleApplyFilters, 0);
                                }}
                            >
                                <div className='flex items-center gap-2'>
                                ...
                                {assigneeId === assignee.id && <Check className='w-4 h-4 text-primary' />}
                            </DropdownMenuItem>
                        ))}
                    </DropdownMenuContent>
                </DropdownMenu>
